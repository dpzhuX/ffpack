...
# Test welchSpectrum
###############################################################################
def test_welchSpectrum_incorrectDataCase_valueError():
    fs = 10

    # case 1: data is a scalar
    data = 10
    with pytest.raises( ValueError ):
        _ = lsm.welchSpectrum( data, fs )
...
def test_welchSpectrum_incorrectFsCase_valueError():
    data = [ 2, 5, 3, 6, 2, 4, 1, 6, 1, 3, 1, 5, 3, 6, 3, 6, 4, 5, 2 ]
...
def test_welchSpectrum_onePeak_samePeaks():
    np.random.seed( 0 )
    gfs = 1000
    fs1 = 10
    T = 10
    ...
    gdata = np.sin( 2 * fs1 * np.pi * t )
    gdata += np.random.randn( len( gdata ) ) * 10**( n0 / 20.0 ) 
    gfreq, gpsd = lsm.welchSpectrum( gdata, gfs )

    ind = np.argpartition( gpsd, -1 )[ -1: ]
    np.testing.assert_allclose( gfreq[ ind[ 0 ] ], fs1, atol=1 )


def test_welchSpectrum_twoPeaks_samePeaks():
    np.random.seed( 0 )
    gfs = 1000
    fs1 = 10
    fs2 = 60
    ...
    peak1 = min( gfreq[ ind ] )
    peak2 = max( gfreq[ ind ] )
    np.testing.assert_allclose( [ peak1, peak2 ], [ fs1, fs2 ], atol=1 )


def test_welchSpectrum_threePeaks_samePeaks():
    np.random.seed( 0 )
    gfs = 1000
    fs1 = 10
    fs2 = 60
    ...
    peak3 = max( gfreq[ ind ] )
    peak2 = sum( gfreq[ ind ] ) - peak1 - peak3
    np.testing.assert_allclose( [ peak1, peak2, peak3 ], [ fs1, fs2, fs3 ], atol=1 )
